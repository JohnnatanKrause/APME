...
function validateForm1() {
    const operadorId = document.getElementById("operadorId").value.trim();
    ...
    if (!operadorId) {
        alert("Por favor, preencha o campo 'Responsável pela Inspeção'.");
        return false;
    }
    if (!tagMotor) {
        alert("Por favor, preencha o campo 'TAG do Motor'.");
        return false;
    }
    if (!marca) {
        alert("Por favor, preencha o campo 'Marca do Motor'.");
        return false;
    }
...
function validateForm2() {
    ...
    if (!modelo) {
        alert("Por favor, selecione o modelo do motor!");
        return false;
    }
    ...
    for (const inputId of numberInputs) {
        ...
        if (isNaN(value)) {
            alert(
                `Por favor, preencha o campo '${document.querySelector(`label[for="${inputId}"]`).textContent
                }' corretamente!`
            );
            return false;
        }
...
function compareMotors() {
    ...
    const temperaturaOperacaoMedido = parseFloat(
        document.getElementById("temperaturaOperacao").value
    );
    const correnteFase1Medido = parseFloat(
        document.getElementById("correnteFase1").value
    );
    const correnteFase2Medido = parseFloat(
        document.getElementById("correnteFase2").value
    );
    const correnteFase3Medido = parseFloat(
        document.getElementById("correnteFase3").value
    );
    ...
    const ruidoDianteiro = document
        .getElementById("ruidoDianteiro")
        .value.toUpperCase();
    const ruidoTraseiro = document
        .getElementById("ruidoTraseiro")
        .value.toUpperCase();
    const setor = document.getElementById("inspeçãoTipo").value;
    ...
    const match = motorData.find(
        (motor) => motor.modelo.toLowerCase() === modelo.toLowerCase()
    );
    ...
    const alertasHTML =
        alertas.length > 0
            ? `<div class='alerta'><h3>ALERTAS:</h3><ul>${alertas
...
function geraTabelaComparacao(
    ...
    const data = [
        {
            description: "Potência (CV)",
            nominal: motor.potencia,
            measured: potenciaMedido,
            parecer: getTechnicalOpinion(motor.potencia, potenciaMedido),
        },
        {
            description: "Tensão Fase R (V)",
            nominal: motor.tensaoNominal,
            measured: tensaoFase1Medido,
            parecer: getTechnicalOpinion(
                motor.tensaoNominal,
                tensaoFase1Medido,
                "tensao",
                "Fase R",
        ...
        {
            description: "Tensão Fase S (V)",
            nominal: motor.tensaoNominal,
            measured: tensaoFase2Medido,
            parecer: getTechnicalOpinion(
                motor.tensaoNominal,
                tensaoFase2Medido,
                "tensao",
        ...
        {
            description: "Tensão Fase T (V)",
            nominal: motor.tensaoNominal,
            measured: tensaoFase3Medido,
            parecer: getTechnicalOpinion(
                motor.tensaoNominal,
                tensaoFase3Medido,
                "tensao",
        ...
        {
            description: "Corrente Fase R (A)",
            nominal: motor.correnteNominal,
            measured: correnteFase1Medido,
            parecer: getTechnicalOpinion(
        ...
        {
            description: "Corrente Fase S (A)",
            nominal: motor.correnteNominal,
            measured: correnteFase2Medido,
            parecer: getTechnicalOpinion(
                motor.correnteNominal,
                correnteFase2Medido,
                "corrente",
        ...
        {
            description: "Corrente Fase T (A)",
            nominal: motor.correnteNominal,
            measured: correnteFase3Medido,
            parecer: getTechnicalOpinion(
                motor.correnteNominal,
                correnteFase3Medido,
                "corrente",
        ...
        {
            description: "Temperatura de Operação (°C)",
            nominal: motor.temperaturaOperacao,
            measured: temperaturaOperacaoMedido,
            parecer: getTemperatureOpinion(
                motor.temperaturaOperacao,
                temperaturaOperacaoMedido,
                alertas
            ),
        },
        {
            ...
            nominal: null,
            measured: vibracao,
            parecer: getVibrationOpinion(vibracao, alertas),
        },
        {
            description: "Ruído Rolamento Dianteiro",
            nominal: null,
            measured: ruidoDianteiro,
            parecer: getNoiseOpinion(ruidoDianteiro, "Dianteiro", alertas),
        },
        {
            description: "Ruído Rolamento Traseiro",
            nominal: null,
            measured: ruidoTraseiro,
            parecer: getNoiseOpinion(ruidoTraseiro, "Traseiro", alertas),
        },
    ];
    ...
    const rows = data
        .map(
            (item) =>
                `<tr><td>${item.description}</td><td>${item.nominal ||
                "*"}</td><td>${item.measured || "*"}</td><td>${item.parecer}</td></tr>`
        )
        .join("");
    let output = `<table><tr><th>Descrição</th><th>Dados Nominais</th><th>Valores Medidos</th><th>Parecer Técnico</th></tr>${rows}</table>`;
    return output;
}

const getTechnicalOpinion = (
    nominal,
    measured,
    type = "other",
    phase = "",
    ...
    let opinion;
    if (type === "tensao") {
        if (diffPercentage <= VOLTAGE_LIMIT) {
        ...
        if (diffPercentage <= CURRENT_LIMIT) {
            opinion = "Condição Normal";
        } else {
            opinion = "Condição Anormal";
            const prefix = measured < nominal ? "Sub" : "Sobre";
...
const getVibrationOpinion = (vibracao, alertas) => {
    const opinion =
        vibracao.toLowerCase() === "sim" ? "Condição Anormal" : "Condição Normal";
...
const getNoiseOpinion = (noise, position, alertas) => {
    const opinion =
        noise.toLowerCase() === "sim" ? "Condição Anormal" : "Condição Normal";
    if (opinion === "Condição Anormal")
        alertas.push(`Ruído detectado no rolamento ${position}`);
    return opinion;
};
...
function printComparison() {
    ...
    printWindow.document.write(
        `<html><head><title>Análise de dados de Motores</title><style>body{font-family:sans-serif;} table { width: 100%; border-collapse: collapse; } th, td { border: 1px solid #ddd; padding: 8px; text-align: left; } th { background-color: #f2f2f2; }</style></head><body>${printContent}</body></html>`
    );
